...
public class SpawnSwordBeamPacket implements IMessage, IMessageHandler<SpawnSwordBeamPacket, IMessage> {
    ...
    public IMessage onMessage(SpawnSwordBeamPacket message, MessageContext ctx) {
        ...
        if (player != null) {
            ...
            int level;
            if ((level = EnchantmentHelper.getEnchantmentLevel(ModEnchantments.sSwordBeam, stack)) > 0 && player.getCooldownTracker().getCooldown(stack.getItem(), 0) == 0) {
                ...
                // TODO 可配置
                player.getCooldownTracker().setCooldown(stack.getItem(), 80 - level * 13);
            }
        }
        return null;
    }
